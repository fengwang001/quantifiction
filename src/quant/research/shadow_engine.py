...
POLL_SEC = 3
PRINCIPAL = 2000.0               # 账户本金（USD，固定）
NOTIONAL = 100.0                 # 基准名义（信心0.5时=本金5%）；实际每单由Agent信心动态定
MAX_TRADE_FRAC = 0.1             # 单笔最大占本金比例（信心1.0时=$200）


def size_from_conviction(conviction: float) -> float:
    """Agent信心→单笔仓位（迭代111）：本金×10%×信心，clamp[$40,$200]。
    信心0.5→$100(与旧基准一致)，信心越高仓位越大。"""
    n = PRINCIPAL * MAX_TRADE_FRAC * max(0.0, min(1.0, conviction))
    return round(max(40.0, min(200.0, n)), 2)
...
class Strategy:
    ...
    def on_tick(self, feat, now):
        ...
        if self.pos == 0:
            ...
            if confirmed and cooled and regime_ok:
                ...
                # Agent信心动态定仓（迭代111）：入场时锁定本单仓位
                self.entry_notional = feat.get("notional", NOTIONAL)
        else:
    ...
    def _close(self, exit_px, gross_pct, held, reason, now):
        ...
        notl = getattr(self, "entry_notional", NOTIONAL)    # 本单仓位(Agent信心定，迭代111)
        net_pct = gross_pct - fee
        net_usd = notl * net_pct
        gross_usd = notl * gross_pct
        fee_usd = notl * fee
        ...
        self.trades.append({
            ...
            "notional": round(notl, 2),   # 本单仓位(Agent信心定，迭代111)
    ...
    def to_dict(self):
        ...
        return {
            ...
            "entry_notional": getattr(self, "entry_notional", NOTIONAL),
    ...
    def load_dict(self, d):
        ...
        self.entry_notional = d.get("entry_notional", NOTIONAL)
...
def make_strategies():
    strats = [
        ...
        # ---- 迭代107（MAE分离）：均值回归赢单MAE中位0.077%(71%<0.15%) vs 亏单中位0.39%(94%>0.15%)。----
        # 止损0.15%可保住71%赢单、提前砍94%亏单(-0.39→-0.15)。赢亏输赢入场分不清，但入场后MAE深浅分得清。
        # 基于高捕获(最优出场)仅sl_pct 0.3%→0.15%，单变量检验超紧止损能否把边际edge推正。
        Strategy("均值回归·紧止", "meanrev1h", "mom", 0.5, 0.008, 7200,
                 cooldown=300, sl_pct=0.0015, author="agent",
                 trail_arm=0.0025, trail_frac=0.25, fee_pct=0.00032),  # 超紧止损0.15%(利用MAE分离)
    ]
...
def main():
    ...
    print(f"影子对比启动：{len(strategies)}个策略 · 扣费{ROUNDTRIP_FEE_PCT*100:.2f}%往返 · "
          f"本金${PRINCIPAL:.0f}·单仓Agent信心定($40-200) · 循环{POLL_SEC}s · 持久化={PERSIST}", flush=True)
    ...
    while True:
        try:
            ...
            # Agent 信心动态定仓（迭代111）：本金$2000，单仓=信心决定，喂给所有策略
            conv = agent_view.get("conviction", 0.5) if agent_view else 0.5
            feat["notional"] = size_from_conviction(conv)
            if agent_view:
                for s in strategies:
            ...
            for s in strategies:
                ...
                notl = getattr(s, "entry_notional", NOTIONAL)   # 本单仓位(Agent信心定)
                fe = getattr(s, "fee_pct", ROUNDTRIP_FEE_PCT)
                # 若此刻平仓的净盈亏（要扣往返费）
                net_if_close = notl * (upnl_pct - fe)
                gross_now = notl * upnl_pct
                if s.pos > 0:
                ...
                open_trades.append({
                    ...
                    "qty": round(notl / s.entry_px, 5),        # 买了多少(ETH)
                    "invested": round(notl, 2),                    # 投入资金(USDT,本单仓位)
                    "cur_value": round(notl * (1 + upnl_pct), 3),  # 现在价值(USDT,含方向)
            ...
            snapshot = {
                ...
                "runtime_sec": runtime,
                "fee_roundtrip_pct": ROUNDTRIP_FEE_PCT * 100,
                "notional": feat.get("notional", NOTIONAL),   # 当前Agent信心定的单仓
                "principal": PRINCIPAL, "conviction": round(conv, 2),
